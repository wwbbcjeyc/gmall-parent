package com.xbgh.gmall.realtime.utils
...
object MyEsUtil {
  private val ES_HOST = "http://192.168.1.101"
  ...
  def insert(source:Any): Unit ={
    ...
    val index: Index = new Index.Builder(source).index("gmall_test").`type`("stud").id("2").build() //代表一次插入动作
    jest.execute(index)
  }
